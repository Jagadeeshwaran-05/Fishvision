...
import { offlineSavedFishApi } from "@/services/offlineStorage";
...
export default function FishDetailsScreen() {
  ...
  const checkSavedStatus = async () => {
    try {
      ...
      const savedFishList = response.data || [];
  ...
  const handleSaveFish = async () => {
    ...
    try {
      ...
      if (!token) {
        Alert.alert(
          "Login Required",
          "Please login to save fishes to your collection",
          [
            { text: "Cancel" },
      ...
      if (isSaved && savedFishId) {
        ...
        await offlineSavedFishApi.deleteSavedFish(savedFishId);
        setIsSaved(false);
        setSavedFishId(null);
        Alert.alert("Removed", "Fish removed from your collection");
      } else {
        // Save the fish
        const result = await offlineSavedFishApi.saveFish({
          fish_name: fish.name,
        ...
        if (result.success && result.data) {
          setIsSaved(true);
          setSavedFishId(result.data.id);
          Alert.alert("Success", "Fish saved to your collection!");
        } else {
          Alert.alert("Info", result.message || "Fish already in collection");
        }
      }
    } catch (error: any) {
