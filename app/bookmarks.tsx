...
import { savedFishApi, savedPlacesApi } from "../services/api";
...
export default function BookmarksScreen() {
  ...
  const loadData = async () => {
    try {
      ...
      if (!userToken) {
        Alert.alert("Not Logged In", "Please log in to view your bookmarks", [
  ...
  const fetchSavedPlaces = async (userToken: string) => {
    try {
      const response = await savedPlacesApi.getSavedPlaces(userToken);
      if (response.success) {
  ...
  const fetchPlaceStats = async (userToken: string) => {
    try {
      const response = await savedPlacesApi.getStats(userToken);
      if (response.success) {
        setPlaceStats({
  ...
  const handleDeletePlace = async (placeId: number) => {
    ...
    Alert.alert("Remove Place", "Are you sure you want to remove this place?", [
      { text: "Cancel", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            await savedPlacesApi.deleteSavedPlace(token, placeId);
